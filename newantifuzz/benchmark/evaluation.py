...
while cnt*parallel_num*execute_one_round < len(parallel_commands):
    ...
    cmd = [cmd[i] for i in range(len(cmd)) if cmd[i] != '']
    #print(len(cmd))
    cmd = '\n'.join(cmd)
    time.sleep(2)
    ...
    process3 = subprocess.Popen(["grep", "honggfuzz "], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    #print(process, ps)
    data, err = process.communicate(ps)
    data2, _ = process2.communicate(ps)
    data3, _ = process3.communicate(ps)
    check_num = len(data.decode().split("\n")) - 1
    check_num += len(data2.decode().split("\n")) - 1
    check_num += (len(data3.decode().split("\n")) - 1) * parallel_num
